...
public:
	...
	bool choque_personasb() {
		if (objsb->getX() >= objp2->getX() && objsb->getX() < objp2->getX() + 6 &&
			objsb->getY() >= objp2->getY() && objsb->getY() < objp2->getY() + 1) {
			return true;
		}
		return false;
	}
	bool choque_personasm() {
		if (objsm->getX() >= objp2->getX() && objsm->getX() < objp2->getX() + 6 &&
			objsm->getY() >= objp2->getY() && objsm->getY() < objp2->getY() + 1) {
			return true;
		}
		return false;
	}
